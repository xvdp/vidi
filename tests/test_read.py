...
def main(src, frames, batch_size, dtype, outtype, asloop, debug):
    ...
    with vidi.FFread(src, batch_size=batch_size, out_type=outtype, start=_st, frames=frames,
                     pix_fmt=_pf, debug=debug, dtype=dtype, device=_dv, grad=_gr) as F:
...
if __name__ == "__main__":
    main(src=args.src, frames=args.frames, batch_size=args.batch_size, dtype=args.dtype,
         outtype=args.outtype, asloop=args.asloop, debug=args.debug)
